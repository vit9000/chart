...
BOOL CMainDlg::OnInitDialog()
{
	...
	//создание панели инструментов дл€ переключени€ режима
	rect.top = 0;
	rect.left = 0;
	...
	m_modeToolBar.addButton(this, L"Ќаркозна€ карта", [this]() {SetAnesth_Mode(); }, 1);
	m_modeToolBar.addButton(this, L"–еанимац.карта", [this]() {SetICU_Mode(); }, 1);
	...
	auto func = [this, index, &main_bridge, &time_type, &date, &visitid](IDBResult& rs)
	{
		...
		*/
		if (charts.size() == 0)
		{
			...
			{
				ChartDB chart;
				if (time_type == TIME_TYPE::ANESTH_CHART) // если наркозна€ карта, то врем€ должно быть текущим. –азве что с точностью до 
				{
...
void CMainDlg::setMode(int MODE)
{
	...
	if(MODE == TIME_TYPE::ICU_CHART)
		...
		config->setTimes(TIME_TYPE::ANESTH_CHART, 180, 10, 10); // в наркозной карте максимальное врем€ сдвинетс€ в зависимости от данных из Ѕƒ
